    ...
    document.getElementById('priceForm').addEventListener('submit', function(event) {
        ...
        function updateCartDisplay() {
            ...
            cart.forEach(item => {
                ...
                cartItemElement.textContent = `${item.title} - ${item.quantity} x $${item.pricePerUnit.toFixed(2)} = $${item.totalPrice.toFixed(2)}`;
                cartList.appendChild(cartItemElement);
                totalCartPrice += item.totalPrice;
            });
            totalPriceElement.textContent = `Total Price: $${totalCartPrice.toFixed(2)}`;
        }
    });
});